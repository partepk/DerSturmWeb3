...
(function H9YL() {
  ...
  const binLoader = (function binLoader() {
    ...
    function fetchBinary(path, fne, msg) {
      if (!loadedBins[fne]) {
        loadedBins[fne] = true;
        fetch(path + fne).then(function resolve(response) {
          if (response.ok) {
            const name = fne.slice(0, fne.lastIndexOf("."));
            if (name === "hyphenEngine") {
              H.binaries[name] = response
                .arrayBuffer()
                .then(function getModule(buf) {
    ...
    return H.clientFeat.wasm ? fetchBinary : requestBinary;
  })();
  ...
  function loadRessources(lang) {
    ...
    if (H.clientFeat.wasm) {
      binLoader(H.paths.maindir, "hyphenEngine.wasm", ["engineLoaded", "wasm"]);
    } else {
    ...
    const tester = (function tester() {
      ...
      function createTest(lang) {
        ...
        testDiv.style.cssText =
          "visibility:hidden;-moz-hyphens:auto;-webkit-hyphens:auto;-ms-hyphens:auto;hyphens:auto;width:48px;font-size:12px;line-height:12px;border:none;padding:0;word-wrap:normal";
